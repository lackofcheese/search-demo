 ...
 * that stores the core inputs of a search: a root state, a goal test, a
 * successor function, and (if appropriate) a heuristic function.
 * 
 * @author lackofcheese
 * @param <S>
...
public abstract class AbstractSearchAlgorithm<S extends State> {
	 ...
	 */
	public AbstractSearchAlgorithm(S root, GoalTest<S> goalTest,
	...
	public void verboseSearch() {
		...
		if (this.goalFound()) {
			...
			System.out.println(String.format(
					"Arrived at %s for cost %.2f at depth %d", goalState,
					finalCost, finalDepth));
	...
	/**
	 * Returns the goal state found in a search; behaviour undefined if no goal
	 * has been found.
	 * 
